...
class Memory_vft(object):
    ...
    def push(self, keys, logits):
        ...
        keys = keys.reshape(len(keys), self.key_dimension)
        for i, key in enumerate(keys):
            key_tensor = torch.from_numpy(key)
            key_flat = key_tensor.flatten().unsqueeze(0) # (1,768)
            key_bytes = key_flat.numpy().tobytes()  # 转换为字节串
            
            if len(self.memory.keys()) >= self.size:
                # Memory is full, find the nearest neighbours and update them
                all_keys = torch.stack([torch.from_numpy(np.frombuffer(k, dtype=np.float32).reshape(1, -1)) for k in self.memory.keys()])
                similarity_scores = torch.matmul(all_keys, key.T) / (
                    torch.norm(all_keys, dim=1, keepdim=True) * torch.norm(key)
                )
                top_k_indices = torch.argsort(similarity_scores.squeeze(-1), descending=True)[:5]
                # all_keys = np.frombuffer(np.asarray(list(self.memory.keys())), dtype=np.float32).reshape(self.size, 1, self.key_dimension)
                # similarity_scores = np.dot(all_keys, key.numpy().T) / (np.linalg.norm(all_keys, axis=1) * np.linalg.norm(key.numpy()))
                # all_keys = torch.stack(list(self.memory.keys()))  # stack all memory keys to tensor
                # similarity_scores = torch.matmul(all_keys, key_flat.T) / (torch.norm(all_keys, dim=1) * torch.norm(key_flat.T,dim=1))
                # Memory is full, find the nearest neighbours and update them
                # all_keys = np.frombuffer(np.asarray(list(self.memory.keys())), dtype=np.float32).reshape(self.get_size(), self.dimension)
                # similarity_scores = np.dot(all_keys, key.T) / (norm(all_keys, axis=1) * norm(key.T))
                # top_k_indices = np.argsort(similarity_scores)[-5:]  # Top-k indices with highest similarity
    ...
    def retrieve_prompt_add_avg(self, avg_pano_embeds, combined, top_k=24): #now for usage of visual feature
        """
