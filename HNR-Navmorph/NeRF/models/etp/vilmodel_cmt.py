...
from NeRF.common.ops import create_transformer_encoder
from NeRF.common.transformer import TransformerEncoder, TransformerEncoderLayer
from NeRF.common.ops import extend_neg_masks, gen_seq_masks, pad_tensors_wgrad
from NeRF.models.etp.nerf import create_nerf, run_nerf_rgb, run_nerf_feature
...
class GaussianSampler(nn.Module):
    def __init__(self, feature_dim):
        ...
        # self.log_sigma = nn.Parameter(torch.zeros(feature_dim))  # 以对数形式参数化，保证 sigma > 0

    def forward(self, mu, sigma, use_sample=True):
        if use_sample:
            # sigma = torch.exp(self.log_sigma)
            # sigma = torch.clamp(sigma, min=1e-3, max=1.0)  # 限制 sigma 的范围
            noise = torch.randn_like(mu)
            sampled = mu + sigma * noise
            return sampled #, mu, sigma #mu + sigma * noise
        else:
            return mu #, mu, sigma
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.W0 = self.global_sap_head.net[4].weight.data
        self.B0 = self.global_sap_head.net[4].bias.data
        self.W0_norm = self.W0.norm(p=2) # calculate norm for weights

        self.pos_encoder = nn.Linear(3, 768)
        self.pos_imagine = nn.Sequential(
            nn.Linear(1536, 768),  # 线性层
            # nn.BatchNorm1d(768),        # 批归一化层
            nn.ReLU(),                         # 激活函数
            nn.Dropout(0.5),                   # Dropout 正则化
            nn.Linear(768, 3)  # 输出层
        )
        ft_dim = 1536
        self.gaussian_sampler_prior = GaussianSampler(ft_dim)
        self.gaussian_sampler_posterior = GaussianSampler(ft_dim)
        # 定义用于生成先验和后验 mu 和 sigma 的层
        self.prior_mu_layer = nn.Linear(ft_dim, ft_dim)
        self.prior_sigma_layer = nn.Linear(ft_dim, ft_dim)
        
        self.posterior_mu_layer = nn.Linear(ft_dim, ft_dim)
        self.posterior_sigma_layer = nn.Linear(ft_dim, ft_dim)
        self.softplus = nn.Softplus(beta=1, threshold=20)
        self.imagine_rgb = PanoramicToRGBDNet_Simple()
        ...
        self.pcd_proj = nn.Sequential(
            nn.Linear(3, 128),
            torch.nn.LayerNorm(128, eps=1e-12)
        )

        self.pcd_position_embedding = nn.Sequential(
            nn.Linear(3, 128),
            torch.nn.LayerNorm(128, eps=1e-12)
        )
        ...
        self.pcd_aggregation = nn.Sequential(
            nn.Linear(args.pointcloud_search_num*128, width),
            torch.nn.LayerNorm(width, eps=1e-12)
        )
    ...
    def forward_panorama(
        self, rgb_fts, loc_fts, nav_types, view_lens, delta_p
    ):
        ...
        # pano_masks = gen_seq_masks(pano_lens * 2)
    ...
    def forward_navigation(
        ...
        gmap_masks, gmap_visited_masks, gmap_pair_dists, grid_fts, grid_map_indexs, gridmap_pos_fts, stepk
    ):

        batch_size = len(grid_fts)
        grid_map_input = torch.zeros(batch_size,8*8,768).to(grid_fts[0].device)
        ...
        grid_map_embeds = self.grid_encoder(grid_map_embeds, src_key_padding_mask=grid_masks.logical_not())
        ...
        if stepk == 0: #new------------------------------------------------------
            with torch.no_grad():
               self.global_sap_head.net[4].weight.data.copy_(self.W0)
               self.global_sap_head.net[4].bias.data.copy_(self.B0) #new---------------------------------------------------
        ...
        gmap_size = gmap_embeds.shape[1]
        map_embeds = torch.cat([gmap_embeds,grid_map_embeds],dim=1)
        map_masks = torch.cat([gmap_masks,grid_masks],dim=1)


        map_embeds = self.global_encoder.encoder(
            txt_embeds, txt_masks, map_embeds, map_masks,
            graph_sprels=graph_sprels
        )
        gmap_embeds = map_embeds[:,:gmap_size]
        ...
        global_sap_head_clone = copy.deepcopy(self.global_sap_head) #new-------------------
        global_sap_head_clone.net[4].weight.data.copy_(self.W0)
        global_sap_head_clone.net[4].bias.data.copy_(self.B0)

        origin_global_logits = global_sap_head_clone(gmap_embeds).squeeze(2)   #new-------------------
        origin_global_logits.masked_fill_(gmap_visited_masks, -float('inf')) #new-----------------
        origin_global_logits.masked_fill_(gmap_masks.logical_not(), -float('inf'))  #new-----------------

       
        """"""
        if stepk % 1 == 0:
            with torch.no_grad(): #new------------------------------------------------------
                W = self.global_sap_head.net[4].weight.data
                B = self.global_sap_head.net[4].bias.data
                max_score, max_idx = torch.max(global_logits, dim=1)
                ...
                #delta = W_update - W
                #B_update = B + (1-impulse) * delta.mean(dim=1) # new
                
                
                # W_norm = W_update.norm(p=2)
                # if W_norm > 0:
                    # W_update = W_update * (W_norm/self.W0_norm) # calibrate length
                    # delta = W_update - W
                    # B_update = B + (1-impulse) * delta.mean(dim=1) # new
                
                    
                self.global_sap_head.net[4].weight.data = W_update
                #self.global_sap_head.net[4].bias.data= B_update #new------------------------------------------------------
    ...
    def forward_nerf(
        self, scene_memory, position, direction, camera_angle_x,H,W,images=None,depths=None,clip_fts=None,is_train=True
    ):
        if is_train:
            loss = run_nerf_rgb(self.nerf_args, self, scene_memory, position, direction, H, W, images=images, depths=depths, is_train=is_train)

            if clip_fts != None:
                loss += run_nerf_feature(self.nerf_args, self, scene_memory, position, direction, 8, 8, clip_fts=clip_fts, is_train=is_train)

            return loss
        else:
            predicted_rgb, predicted_depth = run_nerf_rgb(self.nerf_args, self, scene_memory, position, direction, H, W, images=images, depths=depths, is_train=is_train)
            predicted_fts = run_nerf_feature(self.nerf_args, self, scene_memory, position, direction, 8, 8, clip_fts=clip_fts, is_train=is_train)
            return (predicted_rgb,predicted_depth,predicted_fts)


    def forward_imagine(self, pred_pano_embeds):################################################################################
        # 生成先验的 mu 和 sigma
        mu_prior = self.prior_mu_layer(pred_pano_embeds)
        sigma_prior = self.softplus(self.prior_sigma_layer(pred_pano_embeds)) + 1e-3 # 确保 sigma > 0
        sigma_prior = torch.clamp(sigma_prior, min=1e-3, max=1.0)

        # 使用先验 GaussianSampler 进行采样
        sampled_pred_pano_embeds = self.gaussian_sampler_prior(mu_prior, sigma_prior, use_sample=True)
        # sample_pred_pano_embeds, mu_prior, sigma_prior = self.gaussian_sampler_prior(pred_pano_embeds, use_sample=True)
        pred_delta = self.pos_imagine(sampled_pred_pano_embeds)
        return pred_delta, sampled_pred_pano_embeds, mu_prior, sigma_prior 
    

    def forward_predict(self, pred_pano_embeds):################################################################################
        mu_posterior = self.posterior_mu_layer(pred_pano_embeds)
        sigma_posterior = self.softplus(self.posterior_sigma_layer(pred_pano_embeds)) + 1e-3  # 确保 sigma > 0
        sigma_posterior = torch.clamp(sigma_posterior, min=1e-3, max=1.0)
        sample_pred_pano_embeds = self.gaussian_sampler_posterior(mu_posterior, sigma_posterior, use_sample=True)
        pred_delta = self.pos_imagine(sample_pred_pano_embeds)
        return pred_delta, sample_pred_pano_embeds, mu_posterior, sigma_posterior
