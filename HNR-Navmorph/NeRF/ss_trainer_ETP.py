...
from NeRF.common.base_il_trainer import BaseVLNCETrainer
from NeRF.common.env_utils import construct_envs, construct_envs_for_rl, is_slurm_batch_job
from NeRF.common.utils import extract_instruction_tokens
from NeRF.models.graph_utils import GraphMap, MAX_DIST
from NeRF.utils import reduce_loss
from NeRF.models.utils import get_angle_fts
...
from NeRF.common.utils import dis_to_con, gather_list_and_concat
...
from NeRF.common.ops import pad_tensors_wgrad, gen_seq_masks
...
import sys
sys.path.append('/root/Sim2Real-VLN-3DFF/HNR/utils_p')
...
class RLTrainer(BaseVLNCETrainer):
    def __init__(self, config=None):
        ...
        self.config = config
        train_dir = os.listdir("data/scene_datasets/hm3d/train")
        val_dir = os.listdir("data/scene_datasets/hm3d/val")

        self.hm3d_scenes = {}
        for item in train_dir:
            self.hm3d_scenes[int(item.split('-')[0])] = "data/scene_datasets/hm3d/train/"+item+"/"+item.split('-')[-1]+".basis.glb"
        for item in val_dir:
            self.hm3d_scenes[int(item.split('-')[0])] = "data/scene_datasets/hm3d/val/"+item+"/"+item.split('-')[-1]+".basis.glb"
    ...
    def train(self):
        ...
        for idx in range(start_iter, total_iter, log_every):
            ...
            if self.local_rank < 1:
                ...
                vp_path = '/root/Sim2Real-VLN-3DFF/VLN_3DFF/data/HNR/vpm_1000_wm_im.pkl'
...
"""
    ...
    def image_get_rel_position(self,depth_map,angle,DATASET):
        W=224
        H=224
        half_W = W//2
        half_H = H//2
    ...
    def find_nearest_coord_torch(self, current_coord, gt_path):
        """
        找到 GT 路径中距离 current_coord 最近的坐标。
        
        参数：
            current_coord (torch.Tensor): 当前坐标，形状为 (1, 3)
            gt_path (torch.Tensor): GT 路径坐标序列，形状为 (N, 3)
        
        返回：
            nearest_coord (torch.Tensor): 最近的坐标，形状为 (3,)
            nearest_index (int): 最近坐标的索引
            distance (float): 最近距离
        """
        # 计算欧几里得距离
        distances = torch.norm(gt_path - current_coord, dim=1)  # 形状: (N,)
        # 找到最小距离的索引
        nearest_index = torch.argmin(distances).item() 
        # 获取最近的坐标
        nearest_coord = gt_path[nearest_index]  # 形状: (3,)
        # 获取距离
    ...
    def get_next_coord(self, nearest_index, gt_path_tensor):
        """
        获取 GT 序列中 nearest_index 的下一个坐标。

        参数：
            nearest_index (int): 最近坐标的索引
            gt_path_tensor (torch.Tensor): GT 路径坐标序列，形状为 (N, 3)

        返回：
            next_coord (torch.Tensor or None): 下一个坐标，如果不存在则返回 None
        """
    ...
    def posref_update(self, position, pred_cur_position, ghost_pos, gmap_vp_ids, nav_logits, alpha):
        for idx, vp_id in enumerate(gmap_vp_ids[0]):
            if vp_id in ghost_pos:
                ...
                # nav_probs[:, idx] += 1 / (distance + 1e-6)  # Adjust nav_probs based on distance
                # Use an exponential decay to smooth the weighting adjustment
            else:
                # distance_ref = 0
                distance_ref = np.linalg.norm(pred_cur_position - position)
            weight = np.exp(-alpha * distance_ref)
            # weight = np.clip(1 / (np.sqrt(distance_ref + 1e-6)), a_min=0, a_max=10)
    ...
    def rollout(self, mode, ml_weight=None, sample_ratio=None):
        ...
        vfp_path = '/root/Sim2Real-VLN-3DFF/VLN_3DFF/data/HNR/vpm_1000_wm_im.pkl'
        if os.path.isfile(vfp_path):
            Memory.load_memory(self.memory_vft_pos,vfp_path)

        have_real_pos = True#(mode == 'train' or self.config.VIDEO_OPTION)
        ...
        for stepk in range(self.max_len):
            ...
            self.batch_size = batch_size = len(observations)
            ...
            if stepk == 0:
                ...
                policy_net.pointcloud_x = [[] for i in range(batch_size)]
                policy_net.pointcloud_y = [[] for i in range(batch_size)]
                policy_net.pointcloud_z = [[] for i in range(batch_size)]
                policy_net.pointcloud_rgb = [[] for i in range(batch_size)]
            ...
            new_position = torch.tensor(positions)
            ...
            batch_grid_fts = wp_outputs['batch_grid_fts']
            ...
            # imagine_T = 2
            head_path.append(new_position.clone())  # 直接添加张量
            ...
            if mode == 'train' and stepk !=0: # and stepk % 2==0:
                ...
                for i in range(self.envs.num_envs):
                    ...
                    # 转换为 PyTorch 张量
                    gt_path_tensor = torch.from_numpy(gt_path_1)  # 形状: (N, 3)
                    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                    # 将 GT 路径移动到设备
                    gt_path_tensor = gt_path_tensor.to(device)  # 形状: (N, 3)
                    ...
                    for stepi in range(self.imagine_T):
                        ...
                        # im_path = np.vstack(imagine_path)
            ...
            if mode == 'train' or have_real_pos:#self.config.VIDEO_OPTION:
                cand_real_pos = []
                for i in range(self.envs.num_envs):
                    cand_real_pos_i = [
            ...
            if mode == 'train':
                for b in range(self.batch_size):
                    ...
                    try:
                        selected_nodes = random.choices(list(gmap.keys()), k=4)
                    except:
                        continue

                    selected_fts, (selected_position_x, selected_position_y, selected_position_z), selected_patch_directions, selected_patch_scales, pcd, fcd, pcd_tree, fcd_tree, occupancy_pcd_tree = policy_net.getSceneMemory(b,only_feature=False)
                    ...
                    for key in selected_nodes:
                        ...
                        position = gmap[key][0].tolist()
                        camera_obs = self.envs.call_at(b, "get_observation",{"source_position":position,"source_rotation":rotation})
                        ...
                        subregion_patch_id = []
                        subregion_rgb = []
                        for subregion_id in range(subregion_num):
                            subregion_row = random.uniform(0,1)
                            subregion_col = random.uniform(0,1)
                            subregion_patch_row = int(subregion_row * feature_map_scale)
                            subregion_patch_col = int(subregion_col * feature_map_scale)
                            subregion_patch_id.append(subregion_patch_row * 8 + subregion_patch_col)

                            subregion_row = int(subregion_row * view_rgb.shape[0])
                            subregion_col = int(subregion_col * view_rgb.shape[1])
                            subregion_top = min(max(subregion_row-112,0),224)
                            subregion_down = max(min(subregion_row+111,view_rgb.shape[0]-1),223)
                            subregion_left = min(max(subregion_col-112,0),224)
                            subregion_right = max(min(subregion_col+111,view_rgb.shape[1]-1),223)
                            subregion_rgb.append(np.expand_dims(view_rgb[subregion_top:subregion_down+1,subregion_left:subregion_right+1],axis=0))


                        view_rgb = cv2.resize(view_rgb, (224, 224),  interpolation = cv2.INTER_LINEAR) 
                        view_depth = cv2.resize(view_depth, (224, 224),  interpolation = cv2.INTER_NEAREST)

                        entire_image = np.expand_dims(view_rgb,axis=0)

                        rgb_input = {}
                        rgb_input['rgb'] = torch.tensor(np.concatenate([entire_image]+subregion_rgb,axis=0)).to('cuda')
                        with torch.no_grad():
                            clip_fts = policy_net.rgb_encoder(rgb_input, fine_grained_fts=False)

                        subregion_patch_id = torch.tensor(subregion_patch_id).to(clip_fts.device)

                        view_depth = torch.tensor(view_depth).unsqueeze(0).unsqueeze(-1)
                        view_depth = policy_net.preprocess_depth(view_depth).squeeze(0).numpy()

                        position_dict = {}
                        position_dict['x'] = position[0]
                        position_dict['y'] = position[-1]
                        position_dict['z'] = position[1]
                        ...
                        if self.config.MODEL.task_type == 'r2r':
                            ...
                            loss += policy_net.vln_bert.forward_nerf(scene_memory,(position_dict['x'],position_dict['y'],position_dict['z']), angle,90,224,224,images=view_rgb,depths=view_depth,clip_fts=(clip_fts,subregion_patch_id),is_train=True)
                ...
                for b in range(self.batch_size):
                    ...
                    selected_nodes = list(gmap.keys())[-3:]
                    ...
                    for key in selected_nodes:
                        ...
                        visulize_attention = []
                        position = gmap[key][0].tolist()
                        for angle in tqdm.tqdm(range(12)):
                            angle = -math.pi + math.pi / 6. * angle
                        ...
                        tmp_x = np.concatenate(visulize_x,axis=-1) - position_dict["x"]
                        tmp_y = np.concatenate(visulize_y,axis=-1) - position_dict["y"]
                        tmp_z = np.concatenate(visulize_z,axis=-1) - position_dict["z"]

                        map_x = -(tmp_x * math.cos(angle) + tmp_y * math.sin(angle))
                        map_y = tmp_y * math.cos(angle) - tmp_x * math.sin(angle)
                        map_z = tmp_z

                        map_rgb = np.concatenate([img.reshape((-1,3)) for img in visulize_rgb],axis=0).astype(np.float32) / 255.
                        map_attention = np.concatenate([img.reshape((-1,3)) for img in visulize_attention],axis=0).astype(np.float32) / 255.

                        points_data =  np.concatenate((map_x.reshape((-1,1)),map_y.reshape((-1,1)),map_z.reshape((-1,1))),axis=-1)
                        ...
                        #o3d.io.write_point_cloud(str(key)+'_rgb.xyzrgb', pcd_visulize)

                        pcd_visulize = o3d.geometry.PointCloud()
                        pcd_visulize.points = o3d.utility.Vector3dVector(points_data)
                        pcd_visulize.colors = o3d.utility.Vector3dVector(map_attention)
                        #o3d.io.write_point_cloud(str(key)+'heatmap.xyzrgb', pcd_visulize)
                        '''
            ...
            if sum(dones) > 0:
                for i in reversed(list(range(self.envs.num_envs))):
                    if dones[i]:
                        ...
                        policy_net.pointcloud_x.pop(i)
                        policy_net.pointcloud_y.pop(i)
                        policy_net.pointcloud_z.pop(i)
                        policy_net.pointcloud_rgb.pop(i)
