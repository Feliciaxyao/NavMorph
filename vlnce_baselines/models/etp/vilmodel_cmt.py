...
class PanoramicToRGBDNet_Simple(nn.Module):
    def __init__(self, input_dim=1536, output_dim=512, hidden_dim=1024):
        super(PanoramicToRGBDNet_Simple, self).__init__()
        
        # 定义两层结构
        self.fc1 = nn.Linear(input_dim, hidden_dim)  # 1536 -> 1024
        self.fc2 = nn.Linear(hidden_dim, output_dim)  # 1024 -> 512
        
        # 激活函数
        self.relu = nn.ReLU()
        
        # 层归一化（Layer Normalization）帮助训练稳定
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_navigation(
        ...
        gmap_masks, gmap_visited_masks, gmap_pair_dists, stepk,
        #num_st, #new----------------------------------------------
    ):
        # global branch
        gmap_embeds = gmap_img_fts + \
        ...
        outs = {
            ...
            'origin_global_logits': origin_global_logits, #new-----------------
        }
        return outs
